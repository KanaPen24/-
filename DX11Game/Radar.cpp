...
void CRadar::Update()
{
	...
	for (CGameObj* pObj = m_pScene->GetObj(); pObj; pObj = pObj->GetNext()) {
		if (pObj->GetID() != GOT_OBJECT || !pObj->IsVisible())
			continue;
		m_vPos[m_nObject] = pObj->GetPos();
		float dx = m_vPos[m_nObject].x - vPos.x;
		float dy = m_vPos[m_nObject].y - vPos.y;
		float dz = m_vPos[m_nObject].z - vPos.z;
		if (dx * dx + dy * dy + dz * dz > RADAR_RANGE * RADAR_RANGE)
			continue;
		++m_nObject;
		if (m_nObject >= MAX_OBJECT)
			break;
	}
	XMMATRIX mInvWorld = XMLoadFloat4x4(&m_pPlayer->GetInvWorld());
	XMFLOAT3 vPos2D;
	...
	for (int i = 0; i < m_nObject; ++i) {
		...
		m_vPos2D[i].x = RADAR_POS_X +
			vPos2D.x * (RADAR_RADIUS - BRIGHTSPOT_RADIUS) / RADAR_RANGE;
		m_vPos2D[i].y = RADAR_POS_Y +
			vPos2D.z * (RADAR_RADIUS - BRIGHTSPOT_RADIUS) / RADAR_RANGE;
		m_vPos2D[i].z = (vPos2D.y > fRange) ? 1.0f :
			((vPos2D.y < -fRange) ? -1.0f : 0.0f);
	}
}
